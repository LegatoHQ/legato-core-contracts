...
contract BlueprintV3 is ERC721, IIPRepresentation, ITokenized, AccessControl, IVersioned {
    bool initialized;
    ...
    function setActive(bool _active) public isInitialized onlyRole(DEFAULT_ADMIN_ROLE) {
        require(activated != _active, "BlueprintV3: already set to this activated value");
        activated = _active;
        if (_active) {
            emit Activated();
        } else {
    ...
    // default constructor with no paramrters, passing default values ot the base constructor
    constructor() ERC721("default", "default") {}
    ...
    //     string memory _shortName,
    //     string memory _symbol,
    //     string memory _kindsetSettingsUri,
    //     string memory _fileHash
