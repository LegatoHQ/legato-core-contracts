    ...
    IVersioned
{
    ...
    modifier onlyStoreOwner() {
        if (!hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) && !hasRole(STORE_OWNER_ROLE, _msgSender())) {
            require(false, "Not store owner or admin");
        }
        _;
    }
    ...
    function setIpStatus(address _ip, bool _active) public onlyStoreOwner {
    ...
    function setName(string memory _name) public override onlyStoreOwner {
        ledgerName = _name;
    }
    ...
    function mintLicense(MintLicenseRequest memory req) external override isInitialized onlyActive returns (uint256) {
        ...
        LegatoLicenseV3 licenseContract = LegatoLicenseV3(getAddressManager().getLicenseContract());
        require(address(licenseContract) != address(0), "registryv3 - license not attached ");
        VerifyHelper helper = VerifyHelper(getAddressManager().getVerifyHelper());
        helper.checkLicense(req, address(this));
        LicenseRegistryV2 licReg = LicenseRegistryV2(getAddressManager().getLicenseRegistry());
        ...
        tempLicenseMIntParamsV2 memory temp;
        temp.licenseBlueprint = licReg.getLicenseBlueprintbyId(req.licenseBlueprintId);
        temp.pendingLicRegId = licReg.dispensePurchasedLicenseRunningId();

        temp.newInfo = PurchasedLicenseV3({
            ...
            expiryDate: 0,
            auto_minter: _msgSender(),
            auto_blockNumber: block.number,
            auto_timestamp: block.timestamp,
            auto_chainId: block.chainid,
            auto_paymentId: paymentId,
        ...
        require(paymentId > 0, "payment failed");
        LicenseField[] memory sellerFields = licReg.getSellerFieldsForOptInId(req.optInId);

        uint256 nftId = licenseContract.safeMint(temp.newInfo, req.buyerFields, sellerFields, paymentId);

        temp.newInfo.auto_tokenId = nftId;
        licReg.registerPurchase(temp.pendingLicRegId, temp.newInfo);
    ...
    function mintStoreToken(RoyaltyTokenData memory _token) public isInitialized onlyStoreOwner onlyActive {
    ...
    function getActiveIps() external view returns (address[] memory) {
        uint256 count = ipList.length;
        uint256 activeCount = 0;
        for (uint256 i = 0; i < count; i++) {
            if (BlueprintV3(ipList[i]).activated()) {
                activeCount++;
            }
        }
    ...
    function getInactiveIps() external view returns (address[] memory) {
        uint256 count = ipList.length;
        uint256 activeCount = 0;
        for (uint256 i = 0; i < count; i++) {
            if (!BlueprintV3(ipList[i]).activated()) {
    ...
    function updateOwnerWallet(address _newWallet) external override onlyStoreOwner {
        ...
        _grantRole(STORE_OWNER_ROLE, _newWallet);
        _grantRole(STORE_PAYER_ROLE, _newWallet);
    ...
    function updateAssetMetadataUri(address _asset, string memory _uri, string memory _fileHash)
        external
        onlyStoreOwner
    {
        if (!isChild(_asset)) require(false, "Asset not found in store");
        BlueprintV3(_asset).changeMetadataURI(_uri, _fileHash);
    }
