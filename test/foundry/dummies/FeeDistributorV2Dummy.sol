...
import "contracts/dataBound/FeeDistributor/FeeDistributorDAL.sol";
import "contracts/interfaces/IVersioned.sol";

//enum enabled disabled currency

contract FeeDistributorV2DummyDummy is
    FeeDistributorDAL,
    IFeeDistributor,
    ...
    IVersioned
{
        ...
        initializer
    {
        ...
        _setupRole(LEGATO_ADMIN_ROLE, _legatoAdmin);
    ...
    function newFunctionInV2() public pure returns (bool) {
        return true;
    }
    ...
    function addCurrency(address _currency) public whenNotPaused onlyRole(LEGATO_ADMIN_ROLE) {
        require(_currency != address(0), "empty address");
        _data_addToAllowedCurrenciesList(_currency);
        // allowedCurrenciesList.push(_currency);
    }
