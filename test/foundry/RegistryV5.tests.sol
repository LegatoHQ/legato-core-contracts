...
contract RegistryV5_Test is HelperContract {
    ...
    function setUp() public {
        ...
        // License V3 setup
        LegatoLicenseV3 newLicense = new LegatoLicenseV3();
        vm.label(address(newLicense), "LegatoLicenseV3.Instance");
        newLicense.initialize(DEPLOYER, address(addressManager));
        addressManager.changeContractAddressDangerous("contracts.licenseContract", address(newLicense));
        legatoLicenseV2 = LegatoLicenseV2(address(newLicense));

        // Store Impl V5 setup
        RegistryImplV5 newRegistryImplV5 = new RegistryImplV5();
        vm.label(address(newRegistryImplV5), "RegistryImplV5.Instance");
        addressManager.changeContractAddressDangerous("contracts.registryImplementation", address(newRegistryImplV5));

        // License Registry V2 setup
        LicenseRegistryV2 newLicenseRegistry = new LicenseRegistryV2();
        vm.label(address(newLicenseRegistry), "LicenseRegistryV2.Instance");
        addressManager.changeContractAddressDangerous("contracts.licenseRegistry", address(newLicenseRegistry));
    ...
    function test_withdrawWithBalance_admin_okv5() external {
        assertEq(IVersioned(registry).getVersion(), 5);
    ...
    function test_mintSongWithLicense_getNftLicense() external {
        vm.prank(BOB);
        address songAddress = mintSongBlueprintAs();
        ...
        LicenseRegistryV2 licReg = LicenseRegistryV2(address(licenseRegistry));
        assertEq(licReg.getPurchasedLicenseIdsByOwner(MARY).length, 0);
        assertEq(licReg.getPurchasedLicenseInfosByOwner(MARY).length, 0);
        vm.prank(MARY);
        ...
        PurchasedLicenseV3 memory licenseInfo =
            LegatoLicenseV3(address(addressManager.getLicenseContract())).getLicenseInfo(licenseId);
        ...
        // auto_optinId: req.optInId,
        // auto_batchPaymentId: paymentId,
        // auto_licenseContract: address(licenseContract),
        // auto_licenseContractVersion: licenseContract.getVersion(),
        // auto_tokenId: 0, // will be set once we mint the license token,
        // auto_purchasedLicenseId: temp.pendingLicRegId
        assertEq(licenseInfo.auto_licenseContract, address(legatoLicenseV2));
        assertEq(licenseInfo.auto_licenseContractVersion, 3);
        assertEq(licenseInfo.auto_purchasedLicenseId, licenseId);
        assertEq(licenseInfo.auto_optinId, 1);
        assertEq(licenseInfo.auto_batchPaymentId, 1);
        assertEq(licenseInfo.auto_tokenId, 1);

        LicenseField[] memory seller = legatoLicenseV2.getLicenseFieldsSeller(licenseId);
        assertEq(seller.length, 2);
        LicenseField[] memory buyer = legatoLicenseV2.getLicenseFieldsBuyer(licenseId);
        assertEq(buyer.length, 2);
        LicenseField[] memory autoFields = legatoLicenseV2.getLicenseFieldsAuto(licenseId);
        assertEq(autoFields.length, 0);

        PurchasedLicenseV3 memory infoFromLicReg = licReg.getLicenseInfoFromId(licenseInfo.auto_purchasedLicenseId);

        assertEq(infoFromLicReg.auto_licenseContract, licenseInfo.auto_licenseContract);
        assertEq(infoFromLicReg.auto_minter, licenseInfo.auto_minter);
        assertEq(infoFromLicReg.auto_tokenId, licenseInfo.auto_tokenId);
        assertEq(infoFromLicReg.auto_purchasedLicenseId, licenseInfo.auto_purchasedLicenseId);
        assertEq(infoFromLicReg.auto_optinId, licenseInfo.auto_optinId);
        assertEq(infoFromLicReg.auto_batchPaymentId, licenseInfo.auto_batchPaymentId);
        assertEq(infoFromLicReg.auto_licenseContractVersion, licenseInfo.auto_licenseContractVersion);
        assertEq(infoFromLicReg.auto_blockNumber, licenseInfo.auto_blockNumber);
        assertEq(infoFromLicReg.auto_timestamp, licenseInfo.auto_timestamp);
        assertEq(infoFromLicReg.auto_chainId, licenseInfo.auto_chainId);

        assertEq(licReg.getPurchasedLicenseIdsByOwner(MARY).length, 1);
        assertEq(licReg.getPurchasedLicenseInfosByOwner(MARY).length, 1);

        PurchasedLicenseV3 memory licInfo = licReg.getPurchasedLicenseInfosByOwner(MARY)[0];
        assertEq(licInfo.auto_minter, MARY);
        assertEq(licInfo.auto_blockNumber, block.number);
        assertEq(licInfo.auto_purchasedLicenseId, licenseId);
    ...
    function test_storeSettingsv5() public {
        vm.label(address(this), "RegistryV5_Test");
    ...
    function test_detachStoreNotLastv5() public {
